...
import { getCombinedWeatherDataForLocation, CombinedWeatherData } from './api/dataFetcher';
...
async function mockGeocodingApi(cityName: string): Promise<{ latitude: number, longitude: number, name: string, timezone: string } | null> {
 ...
 */
async function searchCityAndDisplayWeather(cityName: string | null): Promise<void> {
    let lat: number;
    let lon: number;
    let displayedCityName: string;
